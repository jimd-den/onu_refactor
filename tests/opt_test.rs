...
#[test]
fn test_pure_data_leaf_attributes() {
    let options = CompilationOptions::default();
    let env = NativeOsEnvironment::new(options.log_level);
    let codegen = OnuCodegen::new();
    let lexer = Box::new(onu_refactor::adapters::lexer::OnuLexer::new(options.log_level));
    ...
    // Verify MIR flag
    assert!(mir.functions[0].is_pure_data_leaf, "MIR function should be marked as pure data leaf");

    pipeline.codegen.set_registry(pipeline.registry.clone());
    let ir = pipeline.codegen.generate(&mir).expect("Codegen failed");

    // Check for attributes in IR
    assert!(ir.contains("readnone"), "Generated IR should contain 'readnone' attribute");
    assert!(ir.contains("nounwind"), "Generated IR should contain 'nounwind' attribute");
}
