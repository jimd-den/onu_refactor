use crate::domain::entities::mir::{MirProgram, MirFunction};
use crate::domain::entities::types::OnuType;
use crate::application::use_cases::memo_strategies::{MemoStrategy, primitive_memo_strategy::PrimitiveMemoStrategy, compound_memo_strategy::CompoundMemoStrategy};
        ...
        for func in program.functions {
            if Self::is_memoizable(&func) {
                let strategy: Box<dyn MemoStrategy> = match func.return_type {
                    OnuType::I64 | OnuType::Boolean | OnuType::Nothing | OnuType::Ptr => Box::new(PrimitiveMemoStrategy),
                    _ => Box::new(CompoundMemoStrategy),
                };
                let (wrapper, inner) = strategy.create_wrapper_and_inner(func, DEFAULT_MEMO_CACHE_SIZE);
...
mod tests {
    use super::*;
    use crate::domain::entities::mir::{MirArgument, BasicBlock, MirOperand, MirTerminator};

    #[test]
    fn test_compound_memo_strategy_panics_for_tuple_return() {
        let func = MirFunction {
            name: "test_compound".to_string(),
            args: vec![MirArgument { name: "x".to_string(), typ: OnuType::I64, ssa_var: 0 }],
        ...
        let program = MemoPass::run(program);
