...
use crate::application::ports::compiler_ports::{Literal, ParserPort, Token};
use crate::application::use_cases::registry_service::RegistryService;
use crate::domain::entities::ast::{
    Argument, BehaviorHeader, BinOp, Discourse, Expression, ReturnType, TypeInfo,
};
...
impl OnuParser {
    ...
    pub fn scan_headers(
        ...
        while !parser.is_at_end() {
            ...
            if matches!(
                token,
                Some(Token::TheBehaviorCalled | Token::TheEffectBehaviorCalled)
            ) {
                ...
                let sig = BehaviorSignature {
                    input_types: header
                        .takes
                        .iter()
                        .map(|a| a.type_info.onu_type.clone())
                        .collect(),
                    return_type: header.delivers.0.clone(),
                    arg_is_observation: header
                        .takes
                        .iter()
                        .map(|a| a.type_info.is_observation)
                        .collect(),
                };
                ...
                while !parser.is_at_end() {
                    ...
                    if matches!(
                        next,
                        Some(
                            Token::TheModuleCalled
                                | Token::TheShapeCalled
                                | Token::TheBehaviorCalled
                                | Token::TheEffectBehaviorCalled
                        )
                    ) {
                        break;
                    }
                ...
                if let Discourse::Shape {
                    name,
                    fields,
                    behaviors,
                } = parser.parse_shape(registry)?
                {
                    let field_defs: Vec<(String, OnuType)> = fields
                        .iter()
                        .map(|f| (f.name.clone(), f.type_info.onu_type.clone()))
                        .collect();
                    let behavior_sigs = behaviors
                        .iter()
                        .map(|b| {
                            (
                                b.name.clone(),
    ...
    pub fn parse_with_registry(
        ...
        self.log(
            LogLevel::Info,
            &format!("Parsing successful: {} discourse units", discourses.len()),
        );
        Ok(discourses)
    }
}
...
impl ParserInternal {
    ...
    fn parse_discourse(
        &mut self,
        registry: &mut RegistryService,
    ) -> Result<Option<Discourse>, OnuError> {
        while let Some(t) = self.peek() {
            match t {
                ...
                Token::TheBehaviorCalled | Token::TheEffectBehaviorCalled => {
                    return Ok(Some(self.parse_behavior(registry)?));
                }
                _ => {
                    self.advance();
                }
    ...
    fn parse_shape(&mut self, registry: &mut RegistryService) -> Result<Discourse, OnuError> {
        ...
        let name = if let Some(Token::Identifier(n)) = self.advance() {
            ...
            return Err(OnuError::GrammarViolation {
                message: "Expected shape name".into(),
                span: Span::default(),
            });
        };
    ...
    fn parse_behavior_header(
        &mut self,
        registry: &mut RegistryService,
    ) -> Result<BehaviorHeader, OnuError> {
        let is_effect = matches!(self.peek(), Some(Token::TheEffectBehaviorCalled));
        let behavior_keyword = if is_effect {
            Token::TheEffectBehaviorCalled
        } else {
            Token::TheBehaviorCalled
        };
        ...
        let name = if let Some(Token::Identifier(n)) = self.advance() {
            ...
            return Err(OnuError::GrammarViolation {
                message: "Expected behavior name".into(),
                span: Span::default(),
            });
        };
    ...
    fn parse_derivation(&mut self, registry: &mut RegistryService) -> Result<Expression, OnuError> {
        ...
        let name = if let Some(Token::Identifier(n)) = self.advance() {
            ...
            return Err(OnuError::GrammarViolation {
                message: "Expected derivation name".into(),
                span: Span::default(),
            });
        };
    ...
    fn parse_primary(&mut self, registry: &mut RegistryService) -> Result<Expression, OnuError> {
        ...
        match &token {
            ...
            _ => Err(OnuError::GrammarViolation {
                message: format!("Unexpected token in primary: {:?}", token),
                span: Span::default(),
            }),
        }
    }
    ...
    fn parse_type_info(
        &mut self,
        registry: &mut RegistryService,
    ) -> Result<Option<TypeInfo>, OnuError> {
    ...
    fn parse_type_name(&mut self, registry: &mut RegistryService) -> Result<OnuType, OnuError> {
        ...
        if let Some(primitive) = OnuType::from_name(&name) {
            if name == "integer" || name == "i64" {
                if let Some(Token::Identifier(w)) = self.peek() {
                    if w == "with" {
                        self.advance();
                        if let Some(Token::Literal(Literal::Integer(bits))) = self.peek().cloned() {
                            self.advance();
                            if let Some(Token::Identifier(b)) = self.peek() {
                                if b == "bits" {
                                    self.advance();
                                    return Ok(OnuType::WideInt(bits as u32));
                                }
                            }
                        }
                    }
    ...
    fn parse_return_type(
        &mut self,
        registry: &mut RegistryService,
    ) -> Result<ReturnType, OnuError> {
        if self.match_token(Token::Nothing) {
            return Ok(ReturnType(OnuType::Nothing));
        }
        let ti = self
            .parse_type_info(registry)?
            .ok_or_else(|| OnuError::GrammarViolation {
                message: "Strict typing enforced: Missing explicit return type".into(),
                span: Span::default(),
            })?;
    ...
    fn parse_arguments(
        &mut self,
        registry: &mut RegistryService,
    ) -> Result<Vec<Argument>, OnuError> {
        self.log(LogLevel::Trace, "Parsing arguments");
        let mut args = Vec::new();
        while let Some(token) = self.peek() {
            self.log(
                LogLevel::Trace,
                &format!("Arguments loop peeking: {:?}", token),
            );
            if matches!(
                token,
                Token::TheModuleCalled
            ...
            match token {
                Token::Identifier(s) if s == "a" || s == "an" || s == "the" => {
                    ...
                    self.log(
                        LogLevel::Debug,
                        &format!("Parsed argument: {} of type {:?}", name, type_info.onu_type),
                    );
                ...
                _ => {
                    self.log(
                        LogLevel::Trace,
                        &format!("Advancing past non-argument token: {:?}", token),
                    );
                    self.advance();
                }
            }
